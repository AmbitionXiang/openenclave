    ...
    size_t* image_size)
{
    ...
    *image_size = image->elf.image_size;
    if (image->submodule)
        *image_size += image->submodule->image_size;

    /* _calculate_size is invoked before patching the image.
     * At this point, we haven't merge the relocation data
     * from base and module images and apply zero paddings.
     * Therefore, we need to apply the padding here. */
    size_t reloc_size = image->elf.reloc_size;
    if (image->submodule)
        reloc_size += image->submodule->reloc_size;
    *image_size += oe_round_up_to_page_size(reloc_size);

    if ((*image_size & (OE_PAGE_SIZE - 1)) != 0)
    ...
    size_t tls_page_count)
{
     ...
     * Note that the image_info holds the non-padded size. */
    oeprops->image_info.reloc_rva = image->image_size;
    oeprops->image_info.reloc_size = image->reloc_size;
    if (module_image)
    {
        oeprops->image_info.reloc_rva += module_image->image_size;
        oeprops->image_info.reloc_size += module_image->reloc_size;
    }
    ...
    /* Merge the relocation data from both base and module images and
     * apply zero-paddings (to the next page size) */
    void* reloc_data = NULL;
    size_t reloc_size = 0;

    reloc_size = oe_round_up_to_page_size(oeprops->image_info.reloc_size);
    ...
    oeprops->image_info.heap_rva =
        oeprops->image_info.reloc_rva + image->reloc_size;
    ...
    for (uint64_t i = 0; i < number_of_entries; i++)
    {
        ...
        else if (section_data[i].d_tag == DT_NEEDED)
        {
            if (needed_offset)
                OE_RAISE_MSG(
                    OE_UNSUPPORTED_ENCLAVE_IMAGE,
                    "Specifying more than one dependent module is unsupported",
                    NULL);
